...
> 발표를 하다가 알게 됬는데 내가 db에서 찾은걸 그냥 때려박은게 아니라 다음과 같은 과정을 진행했었다.
>
> 해시캣 요구 포맷: 알고리즘:반복횟수:Base64(솔트):Base64(해시) 인데 우리가 찾은건 `[솔트 바이트(Salt)] + [해시 바이트(Hash)]` 이거다.
>
> 그렇기 때문에 `u/+LBBOUnadiyFBsMOoIDPLbUR0rk59kEkPU17itdrVWA/kLMt3w+w==`를 Base64 디코딩을 진행해 원본 데이터로 되돌린 후
>
> 앞의 8바이트가 솔트고 뒤의 32 바이트가 SHA-256 이라는걸 알 수 있으니 따로 나눈 후
>
> 각각 따로 Salt : `u/+LBBOUnac=`, hash : `YshQbDDqCAzy21EdK5OfZBJD1Ne4rXa1VgP5CzLd8Ps=` 이렇게 base64 인코딩을 다시 해주고 
>
> `sha256:600000:u/+LBBOUnac=:YshQbDDqCAzy21EdK5OfZBJD1Ne4rXa1VgP5CzLd8Ps=` 이렇게 조립해서 hashcat을 돌렸었다.
